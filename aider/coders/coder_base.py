...
class CoderBase:
    """Base class containing all Coder implementation.
    
    This class contains all the actual implementation. The Coder class
    in base_coder.py just provides the public interface and inherits from this.
    """
    ...
    def create(
        ...
        **kwargs,
        ...
        """Factory method to create a Coder with proper edit format selection."""
    ...
    def clone(self, **kwargs):
        new_coder = self.__class__.create(from_coder=self, **kwargs)
        return new_coder
    ...
    def get_announcements(self):
        try:
            ...
            announce_version = __version__
            if __version__.endswith(".dev"):
                announce_version = "main"

            announcements_url = f"https://aider.chat/announcements/{announce_version}"
            announce = utils.fetch_content(announcements_url)
            if not announce:
                return res

            url_md = f"[{announcements_url}]({announcements_url})"

            cur_date = time.strftime("%Y-%m-%d")

            SEEN_KEY = "aider_announcements"
            seen_announcements = set(self.io.get_from_persistent_store(SEEN_KEY, []))
            new_announcements = []
            for line in announce.splitlines():
                line = line.strip()
                if not line:
                    continue
                ...
                if not seen_announcements or date not in seen_announcements:
                    new_announcements.append(date)

                if not seen_announcements:
                    res.append(f"Announcements available at {url_md}:")
                elif date not in seen_announcements:
                    res.append(f"**New announcement:**")

                if not seen_announcements or date not in seen_announcements:
                    res.append(f"- {text}")

            seen_announcements.update(new_announcements)
            self.io.set_in_persistent_store(SEEN_KEY, list(seen_announcements))

            return res
        except Exception as err:
            if self.verbose:
                self.io.tool_error(f"Error fetching announcements: {err}")
            return []
    def __init__(
        self,
        main_model,
